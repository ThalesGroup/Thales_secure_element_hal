...
thalesEse_new(struct thalesEse_ctx **c)
{
    ...
    if(_thalesEse_lock_mutex() == 0){ //0 for success else mutex not available
        ...
        ctx->frequency = FREQUENCY;
    #ifdef IRQ_MODE
        ctx->interrupt_gpio_chipset = IRQ_GPIO_CHIP;
        ctx->interrupt_gpio_offset = IRQ_GPIO_OFFSET;
    #endif
...
SE_THALES_EXPORT int
thalesEse_reset(struct thalesEse_ctx *ctx)
{
    int err = -1;
    if (ctx == NULL)
        return err;
...
SE_THALES_EXPORT int
thalesEse_cip(struct thalesEse_ctx *ctx, void *atr, size_t r)
{
    int err = -1;
