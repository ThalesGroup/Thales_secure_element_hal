...
static int
spi_set_speed(struct thalesEse_ctx *ctx)
{
    ...
    status = ioctl(ctx->t1.file_descriptor, SPI_IOC_WR_MAX_SPEED_HZ, &speed);
    if(status < 0)
    {
      error("Could not set SPI speed (WR)...ioctl fail\n");
    }
    status = ioctl(ctx->t1.file_descriptor, SPI_IOC_RD_MAX_SPEED_HZ, &speed);
    if(status < 0)
    {
      error("Could not set SPI speed (RD)...ioctl fail\n");
    }
    warning("SPI HW: Set speed status = %d \n", status);
    if(status == 0)
        debug("freq=%d Hz\n", speed);
    return status;
}
...
int
spi_setup(struct thalesEse_ctx *ctx)
{
...
retry:
    ...
    if (ctx->t1.file_descriptor < 0) {
        error("cannot use %s for spi device, errno = 0x%x\n", ctx->devNode, errno);
        if(errno == -EBUSY || errno == EBUSY) {
            retryCnt++;
            error("Retry open driver - retry cnt : %d\n", retryCnt);
    ...
    return ctx->t1.file_descriptor < 0;
}

int
spi_teardown(struct thalesEse_ctx *ctx)
{
    if (ctx->t1.file_descriptor >= 0)
        if (close(ctx->t1.file_descriptor) < 0)
            warning("failed to close fd to %s, %s.\n", ctx->devNode, strerror(errno));
