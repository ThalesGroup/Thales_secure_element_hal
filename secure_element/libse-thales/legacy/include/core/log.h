...
# define error(fmt, ...)    say(ctx, LOG_ERR fmt, ## __VA_ARGS__)
# define warning(fmt, ...)   say(ctx, LOG_WARN fmt, ## __VA_ARGS__)
# define notice(fmt, ...) say(ctx, LOG_NOTICE fmt, ## __VA_ARGS__)
# define info(fmt, ...)   say(ctx, LOG_INFO fmt, ## __VA_ARGS__)
# ifdef ENABLE_DEBUG
#  define debug(fmt, ...)   say(ctx, LOG_DEBUG fmt, ## __VA_ARGS__)
# else
#  define debug(fmt, ...)   nosay(ctx, LOG_DEBUG fmt, ## __VA_ARGS__)
# endif

void vsay(struct thalesEse_ctx *ctx, const char *fmt, va_list args);

static inline void
check_printf(2, 3)
say(struct thalesEse_ctx *ctx, const char *fmt, ...) {
    va_list args;
...
# define error(fmt, ...)        nosay(ctx, fmt, ## __VA_ARGS__)
# define warning(fmt, ...)      nosay(ctx, fmt, ## __VA_ARGS__)
# define notice(fmt, ...)       nosay(ctx, fmt, ## __VA_ARGS__)
# define info(fmt, ...)         nosay(ctx, fmt, ## __VA_ARGS__)
# define debug(fmt, ...)        nosay(ctx, fmt, ## __VA_ARGS__)
...
static inline void check_printf(2, 3)
    nosay(struct thalesEse_ctx *ctx, const char *fmt, ...) {}
#else
static inline void
nosay(struct thalesEse_ctx *ctx, const char *fmt, ...) {}
#endif

void log_teardown(struct thalesEse_ctx *ctx);
