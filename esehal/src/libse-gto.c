...
SE_GTO_EXPORT int
se_gto_power_on(struct se_gto_ctx *ctx)
{
    info("eSE GTO: se_gto_power_on\n");

    return gpio_set_power_on(ctx);
}

SE_GTO_EXPORT int
se_gto_power_off(struct se_gto_ctx *ctx)
{
    info("eSE GTO: se_gto_power_off\n");

    return gpio_set_power_off(ctx);
}
